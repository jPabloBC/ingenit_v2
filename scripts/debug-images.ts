...
const dropboxToken =
	process.env.NEXT_PUBLIC_DROPBOX_TOKEN || process.env.DROPBOX_TOKEN;
...
async function main() {
	...
	const jobFiles = ((allFiles || []) as StorageFileRow[]).filter(
		(f) => f.metadata?.print_job_id === JOB_ID,
	);
	...
	for (const file of jobFiles) {
		...
		try {
			...
			console.log(
				`  - Link generated successfully: ${response.result.link.substring(0, 50)}...`,
			);
		} catch (err: unknown) {
			const errorMessage =
