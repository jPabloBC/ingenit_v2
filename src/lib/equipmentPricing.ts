...
export const calculateSalePrice = (
	...
	const fallbackCategory = EQUIPMENT_CATEGORIES.find(
		(cat) => cat.id === "otros",
	);
	if (!fallbackCategory) {
		throw new Error(
			'Missing fallback category "otros" in EQUIPMENT_CATEGORIES',
		);
	}
	const category =
		EQUIPMENT_CATEGORIES.find((cat) => cat.id === categoryId) ||
		fallbackCategory;
 ...
 */
export const calculateActualMargin = (
	purchasePrice: number,
	salePrice: number,
): number => {
 ...
 */
export const formatEquipmentPrice = (
	price: number,
	country: string,
): string => {
	const currencyMap: {
	...
	const formatted = price.toLocaleString(currency.locale, options);
	if (country === "Chile") {
