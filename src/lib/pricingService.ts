...
export const getPricingForService = async (
	...
	try {
		...
		// No row for country-specific pricing: fallback to latest active row by service.
		const isNoRows =
		...
		if (isNoRows) {
			...
			if (!fallbackError && fallbackData) {
				return fallbackData as PricingItem;
			}
			if (fallbackError) {
				console.error("Error obteniendo precio fallback:", fallbackError);
			}
			return null;
		}
...
export const getAllPricingForCountry = async (
	country: string,
): Promise<PricingItem[]> => {
	try {
		const { data, error } = await supabase
...
export const calculatePriceFromPricing = (
	...
	const regionMultiplier =
		params.country === "Chile" && params.location
	...
	let unitCosts = 0;
	const breakdown = [
		`Precio base: ${pricing.base_price.toLocaleString("es-CL")} ${pricing.currency}`,
	];
	...
	if (params.meters && pricing.unit_prices.meters) {
		...
		unitCosts += cost;
		breakdown.push(
			`Metros (${params.meters}): ${cost.toLocaleString("es-CL")} ${pricing.currency}`,
		);
	}
	...
	if (params.devices && pricing.unit_prices.devices) {
		...
		unitCosts += cost;
		breakdown.push(
			`Dispositivos (${params.devices}): ${cost.toLocaleString("es-CL")} ${pricing.currency}`,
		);
	}
	...
	if (params.points && pricing.unit_prices.points) {
		...
		unitCosts += cost;
		breakdown.push(
			`Puntos (${params.points}): ${cost.toLocaleString("es-CL")} ${pricing.currency}`,
		);
	}
	...
	if (params.rooms && pricing.unit_prices.rooms) {
		...
		unitCosts += cost;
		breakdown.push(
			`Habitaciones (${params.rooms}): ${cost.toLocaleString("es-CL")} ${pricing.currency}`,
		);
	}
	...
	if (params.floors && pricing.unit_prices.floors) {
		...
		unitCosts += cost;
		breakdown.push(
			`Pisos (${params.floors}): ${cost.toLocaleString("es-CL")} ${pricing.currency}`,
		);
	}
	...
	breakdown.push(
		`Subtotal: ${subtotal.toLocaleString("es-CL")} ${pricing.currency}`,
	);

	if (totalMultiplier !== 1.0) {
		breakdown.push(
			`Multiplicador (complejidad + regional): ${totalMultiplier.toFixed(2)}x`,
		);
	}

	breakdown.push(
		`Total: ${totalPrice.toLocaleString("es-CL")} ${pricing.currency}`,
	);
