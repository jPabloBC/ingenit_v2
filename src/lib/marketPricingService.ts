...
const _exchangeRateCache = new Map<string, number>();
...
const _EXCHANGE_API_KEY = process.env.NEXT_PUBLIC_EXCHANGE_API_KEY || "demo";
const _ELECTRONICS_API_KEY = process.env.ELECTRONICS_API_KEY || "";
	...
	try {
		...
		for (const component of components) {
			...
			if (marketPrice) {
				...
				componentPriceCache.set(component, {
 ...
 */
const getComponentPriceFromAPI = async (
	component: string,
...
export const updatePricingFromMarket = async (): Promise<void> => {
	try {
		...
		const [_exchangeRates, componentPrices] = await Promise.all([
			getExchangeRates(),
		...
		for (const componentPrice of componentPrices) {
			...
			if (error) {
				console.error(
					`Error actualizando precio para ${componentPrice.component}:`,
					error,
				);
			}
		}
