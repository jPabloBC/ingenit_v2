...
export const NETWORK_COMPONENTS: TIServiceComponent[] = [
	...
	},
	{
		...
		description:
			"Firewall de nivel avanzado con características de seguridad avanzadas",
		category: "dispositivos",
		subcategory: "firewalls",
		unit: "dispositivo",
		basePrice: 250000,
...
const requiredNetworkComponent = (id: string): TIServiceComponent => {
	const component = NETWORK_COMPONENTS.find((c) => c.id === id);
	if (!component) {
		throw new Error(`Missing network component: ${id}`);
	}
	return component;
};
...
export const NETWORK_PACKAGES: TIServicePackage[] = [
	// PAQUETE BÁSICO - RED PEQUEÑA
	{
		...
		category: "redes",
		components: [
			requiredNetworkComponent("cable_utp_cat6_metro"),
			requiredNetworkComponent("punto_red_rj45"),
			requiredNetworkComponent("switch_24puertos_basico"),
			requiredNetworkComponent("configuracion_switch_basica"),
		],
	...
	// PAQUETE ESTÁNDAR - RED MEDIANA
	{
		...
		category: "redes",
		components: [
			requiredNetworkComponent("cable_utp_cat6a_metro"),
			requiredNetworkComponent("punto_red_rj45"),
			requiredNetworkComponent("patch_panel_24puertos"),
			requiredNetworkComponent("switch_48puertos_basico"),
			requiredNetworkComponent("router_empresarial"),
			requiredNetworkComponent("configuracion_switch_avanzada"),
			requiredNetworkComponent("configuracion_router"),
		],
		totalPrice: 0, // Se calcula dinámicamente
		isActive: true,
		complexity: "standard",
	...
	// PAQUETE AVANZADO - RED EMPRESARIAL
	{
		...
		components: [
			requiredNetworkComponent("cable_utp_cat6a_metro"),
			requiredNetworkComponent("cable_fibra_monomodo_metro"),
			requiredNetworkComponent("punto_red_rj45"),
			requiredNetworkComponent("punto_red_fibra"),
			requiredNetworkComponent("patch_panel_48puertos"),
			requiredNetworkComponent("switch_48puertos_poe"),
			requiredNetworkComponent("router_empresarial"),
			requiredNetworkComponent("firewall_avanzado"),
			requiredNetworkComponent("access_point_interior"),
			requiredNetworkComponent("controlador_wifi"),
			requiredNetworkComponent("configuracion_switch_avanzada"),
			requiredNetworkComponent("configuracion_router"),
			requiredNetworkComponent("configuracion_firewall"),
			requiredNetworkComponent("configuracion_access_point"),
		],
		totalPrice: 0, // Se calcula dinámicamente
		isActive: true,
		complexity: "advanced",
	...
	// PAQUETE ENTERPRISE - RED COMPLETA
	{
		...
		components: [
			...
			requiredNetworkComponent("firewall_avanzado"),
			requiredNetworkComponent("access_point_interior"),
			requiredNetworkComponent("access_point_exterior"),
			requiredNetworkComponent("controlador_wifi"),
			requiredNetworkComponent("camara_ip_domo"),
			requiredNetworkComponent("camara_ip_bullet"),
			requiredNetworkComponent("nvr_16canales"),
			requiredNetworkComponent("configuracion_switch_avanzada"),
			requiredNetworkComponent("configuracion_router"),
			requiredNetworkComponent("configuracion_firewall"),
			requiredNetworkComponent("configuracion_access_point"),
			requiredNetworkComponent("configuracion_camara_ip"),
			requiredNetworkComponent("monitoreo_red_mensual"),
			requiredNetworkComponent("backup_mensual"),
		],
		totalPrice: 0, // Se calcula dinámicamente
		isActive: true,
...
export const getComponentsByCategory = (
	...
	return NETWORK_COMPONENTS.filter(
		(component) => component.category === category && component.isActive,
	);
};
 ...
 */
export const getComponentsBySubcategory = (
	subcategory: string,
): TIServiceComponent[] => {
	return NETWORK_COMPONENTS.filter(
		(component) => component.subcategory === subcategory && component.isActive,
	);
};
 ...
 */
export const getPackagesByComplexity = (
	complexity: string,
): TIServicePackage[] => {
	return NETWORK_PACKAGES.filter(
		(package_) => package_.complexity === complexity && package_.isActive,
	);
};
...
export const generateQuoteFromPackage = (
	...
	const defaultConfigurations: TIServiceConfiguration[] =
