...
function validateRedirectURI(uri: string): ValidationResult {
	...
	try {
		...
		if (url.hash) {
			valid = false;
			details.push(
				"Las URIs de redireccionamiento no deben contener fragmentos (#)",
			);
		}
	...
	return {
		valid,
		message: valid
			? "URI de redireccionamiento válida"
			: "URI de redireccionamiento inválida",
		details,
	};
}
	...
	return (
		<>
			...
			<main className="min-h-screen bg-white text-gray2 font-body px-6 py-16 max-w-4xl mx-auto mt-10">
				...
				<p className="mb-6">
					Esta herramienta te permite validar URIs de redireccionamiento para
					aplicaciones OAuth y servicios de autenticación.
				</p>
				...
				<div className="bg-white border border-gray-200 rounded-lg shadow-lg p-6 mb-8">
					...
					{result && (
						<div className="mt-6">
							<div
								className={`${result.valid ? "bg-green-50 border-green-200" : "bg-red-50 border-red-200"} border rounded-lg p-4`}
							>
								<div className="flex items-start">
									<div
										className={`${result.valid ? "text-green-400" : "text-red-400"} mr-3 mt-1`}
									>
										{result.valid ? "✓" : "✗"}
									</div>
									<div className="flex-1">
										<p
											className={`${result.valid ? "text-green-700" : "text-red-700"} font-medium mb-2`}
										>
											{result.message}
										</p>
										<div
											className={`text-sm ${result.valid ? "text-green-600" : "text-red-600"}`}
										>
				...
				<div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
					<div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
						<h3 className="text-lg font-semibold text-blue2 mb-3">
							✅ URIs Válidas
						</h3>
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-6">
						<div>
							...
							<div className="space-y-2 text-sm font-mono">
								...
								<div className="bg-white p-2 rounded border">
									https://app.ejemplo.cl/auth/redirect
								</div>
								<div className="bg-white p-2 rounded border">
									https://secure.domain.com/login
								</div>
							</div>
						</div>
						...
						<div>
							...
							<div className="space-y-2 text-sm font-mono">
								<div className="bg-white p-2 rounded border">
									http://miapp.com/oauth
								</div>
								<div className="bg-white p-2 rounded border">
									https://localhost:3000/callback
								</div>
								<div className="bg-white p-2 rounded border">
									https://app.com/auth#fragment
								</div>
							</div>
						</div>
					</div>
				...
				<div className="bg-white border border-gray-200 rounded-lg p-6">
					...
					<div className="space-y-4 text-sm">
						<div>
							<h4 className="font-medium text-blue3">
								1. Configuración en OAuth Provider
							</h4>
							<p className="text-gray-600">
								Registra las URIs exactas en tu proveedor OAuth (Google,
								Facebook, etc.)
							</p>
						</div>
						...
						<div>
							...
							<p className="text-gray-600">
								Siempre valida las URIs en el servidor antes de redirigir
							</p>
						</div>
						...
						<div>
							...
							<p className="text-gray-600">
								Mantén una lista blanca de URIs permitidas en tu aplicación
							</p>
						</div>
						...
						<div>
							...
							<p className="text-gray-600">
								Usa siempre HTTPS en producción para proteger los tokens
							</p>
						</div>
					</div>
				</div>
