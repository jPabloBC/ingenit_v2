"use client";
import { Bed, Building, Calendar, Hotel, Star, Users } from "lucide-react";
...
export default function HLAdminPage() {
	...
	const loadHLData = useCallback(async () => {
		try {
        ...
        `)
			...
			if (adminUsersError) {
				console.error(
					"❌ Error cargando usuarios administradores:",
					adminUsersError,
				);
				// Si es error de esquema no encontrado, mostrar mensaje informativo
				if (
					adminUsersError.code === "PGRST116" ||
			...
			if (businessError) {
				console.warn(
					"⚠️ Error cargando datos de negocio (continuando):",
					businessError,
				);
			}
			...
			const { data: reservationsData, error: reservationsError } =
				await supabase
					...
					.select(
						"id, business_id, status, payment_status, total_amount, created_at, created_by",
					)
					.limit(100);

			console.log("📋 Resultado reservas:", {
				reservationsData,
				reservationsError,
			});
			...
			if (reservationsError) {
				console.warn(
					"⚠️ Error cargando reservas (continuando):",
					reservationsError,
				);
			}
			...
			const { data: subscriptionsData, error: subscriptionsError } =
				await supabase
					...
					.select(
						"id, user_id, business_id, plan_id, status, amount, payment_status, created_at",
					)
					.limit(50);

			console.log("📋 Resultado subscriptions:", {
				subscriptionsData,
				subscriptionsError,
			});
			...
			if (subscriptionsError) {
				console.warn(
					"⚠️ Error cargando suscripciones (continuando):",
					subscriptionsError,
				);
			}
			...
			const usersWithData =
				adminUsers?.map((admin) => {
					const userBusiness = businessData?.find(
						(b) => b.user_id === admin.id,
					);
					const userSubscription = subscriptionsData?.find(
						(s) => s.user_id === admin.id,
					);

					// Huéspedes creados por este usuario
					const userGuests =
						guestsData?.filter((g) => g.created_by === admin.id) || [];

					// Reservas de este usuario (por business_id)
					const userReservations =
						reservationsData?.filter(
							(r) => userBusiness && r.business_id === userBusiness.id,
						) || [];
		...
		<div className="p-4 sm:p-6 dashboard-transition">
						...
						<div className="flex items-center">
							...
							<div className="ml-3">
								...
								<div className="mt-2 text-sm text-blue-700">
									<p>
										El sistema hotelero está configurado correctamente, pero no
										hay datos registrados. Para comenzar, crea usuarios
										administradores, negocios, huéspedes y reservas.
									</p>
								</div>
							</div>
						</div>
				...
				<div className="dashboard-card bg-white p-4 sm:p-6 shadow-lg border border-gray-100">
					<div className="flex items-center justify-between">
						<div className="min-w-0 flex-1">
							...
							<p className="text-2xl sm:text-3xl font-bold text-green-600 stats-number">
				...
				<div className="dashboard-card bg-white p-4 sm:p-6 shadow-lg border border-gray-100">
					<div className="flex items-center justify-between">
						<div className="min-w-0 flex-1">
							...
							<p className="text-2xl sm:text-3xl font-bold text-orange-500 stats-number">
				...
				<div className="dashboard-card bg-white p-4 sm:p-6 shadow-lg border border-gray-100">
					<div className="flex items-center justify-between">
						<div className="min-w-0 flex-1">
							...
							<p className="text-2xl sm:text-3xl font-bold text-blue-600 stats-number">
					...
					<div className="space-y-3">
						<div className="flex justify-between items-center">
							...
							<span className="font-semibold text-blue-600">
								{dashboardData.healthMetrics.avgHeartRate}
							</span>
						</div>
						<div className="flex justify-between items-center">
							...
							<span className="font-semibold text-yellow-600">
								{dashboardData.healthMetrics.avgBloodPressure}
							</span>
						</div>
						<div className="flex justify-between items-center">
							<span className="text-sm text-gray-600">Total de Reservas</span>
							<span className="font-semibold text-green-600">
								{dashboardData.healthMetrics.totalCheckups}
							</span>
						</div>
					</div>
				</div>
							...
							>
								...
								<div className="flex-1 min-w-0">
									...
									<p className="text-xs text-gray-500">
										{user.health_data.blood_pressure} •{" "}
										{user.health_data.heart_rate} reservas •{" "}
										{user.health_data.weight} activos
									</p>
								</div>
								<div
			...
			<div className="bg-white rounded-md shadow-lg border border-gray-100 p-4 sm:p-6">
				...
				<div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
					<button
						type="button"
						className="w-full flex items-center gap-3 p-3 text-left hover:bg-gray-50 rounded-md transition-colors duration-200 mobile-button touch-optimized"
					>
