	...
	const savePricingItem = async (item: Partial<PricingItem>) => {
		try {
			if (editingItem) {
				...
				// Crear nuevo item
				const { error } = await supabase.from("rt_pricing_library").insert({
					...item,
	...
	const getFilteredItems = () => {
		return pricingItems.filter((item) => {
			const matchesCountry =
				selectedCountry === "Todos" || item.country === selectedCountry;
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="bg-white rounded-lg shadow-sm p-6 mb-6">
					<div className="grid grid-cols-1 md:grid-cols-4 gap-4">
						<div>
							<label
								htmlFor="pricing-country-filter"
						...
						<div>
							<label
								htmlFor="pricing-category-filter"
								className="block text-sm font-medium text-gray-700 mb-2"
							>
								Categoría
							</label>
				...
				<div className="bg-white rounded-lg shadow-sm">
					...
					<div className="p-4 bg-blue-50 border-b">
						<div className="text-sm text-blue8 space-y-1">
							...
							<div>
								• Mostrando: {getFilteredItems().length} precios filtrados
							</div>
						</div>
					</div>
										...
										<tr key={item.id} className="hover:bg-gray-50">
												...
												<div className="flex items-center gap-2">
													...
													<span className="text-sm font-medium text-gray-900">
														...
														item.base_price > 0
															...
															: Object.entries(item.unit_prices || {})
																	.map(
																		([key, value]) =>
											...
											<td className="px-6 py-4">
												<div className="text-sm text-gray-900">
													{Object.entries(item.unit_prices).map(
														([key, value]) => (
							...
							{pricingItems.length === 0 && (
								<div className="p-8 text-center text-gray-500">
									...
									<p className="text-gray-600 mb-4">
										Parece que no hay precios en la base de datos. Esto puede
										deberse a:
									</p>
		...
		<div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
			<div className="bg-white rounded-lg p-6 w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
				...
				<div className="space-y-4">
					...
					<div>
						<label
							htmlFor="pricing-modal-service"
							className="block text-sm font-medium text-gray-700 mb-2"
						>
							Servicio *
						</label>
						<select
							id="pricing-modal-service"
					...
					<div>
						<label
							htmlFor="pricing-modal-country"
							className="block text-sm font-medium text-gray-700 mb-2"
						>
							País *
						</label>
						<select
							id="pricing-modal-country"
					...
					<div>
						<label
							htmlFor="pricing-modal-currency"
							className="block text-sm font-medium text-gray-700 mb-2"
						>
							Moneda
						</label>
						<input
							id="pricing-modal-currency"
							type="text"
					...
					<div>
						<label
							htmlFor="pricing-modal-base-price"
							className="block text-sm font-medium text-gray-700 mb-2"
						>
							Precio Base *
						</label>
						<input
							id="pricing-modal-base-price"
							type="number"
					...
					{getSelectedService() && (
						<div>
							...
							<div
								id="pricing-unit-price-group"
								className="grid grid-cols-2 gap-4"
							>
								{getSelectedService()?.unit_fields.map((field) => (
									<div key={field}>
										<label
											htmlFor={`pricing-unit-${field}`}
											className="block text-xs text-gray-600 mb-1"
										>
											{field.charAt(0).toUpperCase() + field.slice(1)}
										</label>
										<input
											id={`pricing-unit-${field}`}
											type="number"
