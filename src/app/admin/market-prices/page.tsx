...
import {
	getLocalPricingData,
	updatePricingFromLocal,
} from "@/lib/localPricingService";
...
export default function MarketPricesPage() {
	...
	const [_filteredData, _setFilteredData] = useState<PricingItem[]>([]);
	const [searchTerm, setSearchTerm] = useState("");
	const [_selectedCategory, _setSelectedCategory] = useState("");
	const [_selectedCountry, _setSelectedCountry] = useState("");
	const [_isLoading, _setIsLoading] = useState(true);
	const [_editingItem, setEditingItem] = useState<PricingItem | null>(null);
	const [_priceHistory, setPriceHistory] = useState<PriceTrend[]>([]);
		...
		try {
			...
			const pricingItems: PricingItem[] =
				data.prices?.map((price) => {
					const priceData = price as Record<string, unknown>;
					return {
						id: (String(priceData.component || priceData.id) ||
							Math.random().toString()) as string,
						name: (priceData.component ||
							priceData.name ||
							"Componente") as string,
						category: (priceData.category || "general") as string,
						base_price: Number(priceData.price || 0),
						country: (priceData.country || "Chile") as string,
						region: priceData.region as string | undefined,
	...
	const getFilteredTrends = () => {
		if (!pricingData) return [];
		if (!searchTrends.trim())
			return pricingData.map((item) => [String(item.name), item.base_price]);
		...
		return pricingData
			.filter((item) =>
				String(item.name)
					.toLowerCase()
					.replace(/_/g, " ")
					.includes(searchTrends.toLowerCase()),
			)
	...
	const _getPriceTrends = async (_itemId: string) => {
		try {
	...
	const _handleEdit = (item: PricingItem) => {
		setEditingItem(item);
	};
			...
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
				...
				<div className="mt-8 bg-blue-50 rounded-lg p-6">
					...
					<div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm text-blue-800">
						<div>
							<p>
								<strong>• Actualización automática:</strong> Cada hora
							</p>
							<p>
								<strong>• Fuente de datos:</strong> Investigación de mercado
							</p>
							<p>
								<strong>• Variación:</strong> ±10% para simular mercado real
							</p>
						</div>
						<div>
							<p>
								<strong>• Componentes:</strong> 80+ servicios de desarrollo y TI
							</p>
							<p>
								<strong>• Multiplicadores:</strong> Por región y país
							</p>
							<p>
								<strong>• Tendencias:</strong> Análisis de 6 meses
							</p>
						</div>
					</div>
				</div>
