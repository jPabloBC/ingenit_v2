"use client";
import { CornerUpLeft, Plus, RefreshCw } from "lucide-react";
import { useRouter } from "next/navigation";
import { useCallback, useEffect, useState } from "react";
import type { PrintJob } from "@/lib/printJobs";
import { createPrintJob, deletePrintJob, listPrintJobs } from "@/lib/printJobs";
...
function _getShortUuid(id: string) {
	return id?.split("-")[0] || "";
}
...
export default function PrintImagePage() {
	...
	const handleCreateJob = async () => {
		...
		try {
			...
			setError(`Error creando documento: ${getErrorMessage(err)}`);
		} finally {
			setLoading(false);
		}
	};
	...
	const handleRefresh = useCallback(async () => {
		...
		try {
			...
			setError(`Error cargando documentos: ${getErrorMessage(err)}`);
		} finally {
			setLoading(false);
		}
	}, []);
	...
	const handleDelete = async (id: string) => {
		const ok = confirm(
			"¿Eliminar documento? Esta acción no se puede deshacer.",
		);
		if (!ok) return;
		setError("");
		setLoading(true);
		try {
			...
			setError(`Error eliminando documento: ${getErrorMessage(err)}`);
		} finally {
			setLoading(false);
		}
	};
	...
	return (
		<div className="max-w-full mx-auto p-4 sm:p-8">
			<div className="flex items-center justify-between mb-4">
				<h1 className="text-xl font-normal text-gray7 md:text-3xl">
					Creador de Documentos para Impresión
				</h1>
			</div>
			...
			<div className="mb-6">
				<div className="flex items-center justify-between">
					<p className="block mb-1 font-semibold">Documentos existentes:</p>
				</div>
				...
				<div className="block sm:hidden">
					{jobs.length === 0 && !loading ? (
						<div className="text-sm text-gray-600 mt-2">
							No hay documentos creados todavía.
						</div>
					) : (
							...
							{jobs.map((job) => (
								...
								>
									<div className="flex items-center justify-between">
										<div>
											<a
												href={`/admin/print/image/${job.id}`}
												className="text-blue-700 hover:underline font-mono text-lg font-semibold truncate"
											>
										...
										<span
											className={`px-2 py-0.5 rounded text-xs ${job.status === "done" ? "bg-green-100 text-green-800" : job.status === "error" ? "bg-red-100 text-red-800" : "bg-yellow-100 text-yellow-800"}`}
										>
											{job.status || "pending"}
										</span>
									</div>
						...
						<div className="mt-2 border rounded divide-y">
							...
							{jobs.map((job) => (
								...
								>
									...
									<div className="col-span-4 break-all text-xs text-gray-400">
										{job.id}
									</div>
