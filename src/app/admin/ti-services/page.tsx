...
import {
	...
	type TIServiceComponent,
	type TIServiceConfiguration,
	type TIServicePackage,
	type TIServiceQuote,
...
export default function TIServicesPage() {
	...
	const [selectedPackage, setSelectedPackage] =
		useState<TIServicePackage | null>(null);
	...
	/* biome-ignore lint/correctness/useExhaustiveDependencies: initial load flow intentionally runs once */
	useEffect(() => {
		setIsClient(true);
		void loadData();
		...
		try {
			...
			if (
				serviceData.granularComponents &&
				serviceData.granularComponents.length > 0
			) {
				console.log(
					"🔧 Cargando componentes granulares existentes:",
					serviceData.granularComponents,
				);

				// Crear configuraciones basadas en los componentes existentes
				const existingConfigurations = serviceData.granularComponents.map(
					(comp: Record<string, unknown>) => {
						const unitPrice = Number(comp.unitPrice || comp.price || 0);
						const quantity = Number(comp.quantity || 1);
						return {
							componentId: String(comp.id || comp.name || ""),
							quantity: quantity,
							unitPrice: unitPrice,
							totalPrice: unitPrice * quantity, // Calcular el totalPrice
							name: String(comp.name || ""),
							category: String(comp.category || ""),
							subcategory: String(comp.subcategory || ""),
						};
					},
				);
				...
				console.log(
					"✅ Configuraciones cargadas desde servicio existente:",
			...
			if (serviceData.packageId) {
				console.log(
					"🔧 Cargando datos de paquete existente:",
	...
	async function loadData() {
		...
		try {
			...
			if (!error && pricing) {
				...
				baseComponents = baseComponents.map((c) => {
					const mapped =
						priceMap.get(c.id) ?? priceMap.get(aliasMap.get(c.id) || "");
			...
			console.warn(
				"No se pudo leer pricing_library, se usan precios locales",
				e,
			);
		}
		...
		const compMap = new Map(
			baseComponents.map((c) => [c.id, c] as [string, TIServiceComponent]),
		);
	...
	const addComponentToQuote = (
		component: TIServiceComponent,
		quantity: number = 1,
	) => {
	...
	const addPackageToQuote = (package_: TIServicePackage) => {
		...
		console.log(
			`Paquete "${package_.name}" agregado con ${package_.components.length} componentes`,
		);
	};
	...
	const removeComponentFromQuote = (componentId: string) => {
		setConfigurations(
			configurations.filter((config) => config.componentId !== componentId),
		);
	};
	...
	const generateQuoteFromSelectedPackage = () => {
		...
		console.log(
			`Paquete "${selectedPackage.name}" seleccionado. Haz clic en "Agregar Componentes del Paquete" para personalizar.`,
		);
	};
	...
	const continueToQuote = async () => {
		...
		if (configurations.length > 0 || selectedPackage) {
			...
			if (selectedPackage && configurations.length > 0) {
				...
				const packageQuote = generateQuoteFromPackage(
					selectedPackage.id,
					configurations,
				);
				tiQuoteData = {
				...
				const packageConfigurations = selectedPackage.components.map(
					(component) => ({
						componentId: component.id,
						quantity: 1,
						unitPrice: component.basePrice,
			...
			if (editMode === "true" && quoteId) {
				...
				if (quoteFormData) {
					...
					if (originalService) {
						...
						const updatedServices = formData.selected_services.map(
							(service: Record<string, unknown>) =>
								service.id === original.id
									? {
											...service,
											price: tiQuoteData.totalAmount,
											granularComponents: tiQuoteData.components,
										}
									: service,
						);
						...
						const updatedFormData = {
							...
							total_amount:
								updatedServices.reduce(
									(total: number, service: Record<string, unknown>) =>
										total + Number(service.price || 0),
									0,
								) + Number(formData.equipment_total || 0),
						};
						...
						sessionStorage.setItem(
							"tiQuoteData",
							JSON.stringify(tiDataForDetection),
						);
						console.log(
							"✅ DATOS TI GUARDADOS PARA DETECCIÓN AUTOMÁTICA:",
							tiDataForDetection,
						);
						...
						try {
							console.log(
								"🔄 ACTUALIZANDO COTIZACIÓN EN BASE DE DATOS:",
								quoteId,
							);
			...
			console.log("🔒 PARÁMETROS DE EDICIÓN PRESERVADOS:", {
				editMode,
				quoteId,
			});
			...
			alert(
				"Por favor selecciona al menos un componente o un paquete antes de continuar.",
			);
		}
	};
				...
				<div className="mb-6 sm:mb-8">
					<div className="flex items-center gap-4 mb-4">
						...
						<div className="flex-1">
							...
							{isClient &&
								(() => {
									try {
										const selectedTIService =
											sessionStorage.getItem("selectedTIService");
										if (selectedTIService) {
					...
					<div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
						<p className="text-sm text-blue-800">
							<strong>Paso 2 de 3:</strong> Configuración detallada de servicios
							TI. Una vez que hayas seleccionado todos los componentes
							necesarios, haz clic en "Continuar con Cotización" para proceder
							al paso final.
						</p>
					</div>
				</div>
							...
							<div className="px-6 py-4 border-b border-gray-200">
								...
								{!selectedPackage && (
									<p className="text-sm text-orange-600 mt-1">
										⚠️ Primero debes seleccionar un paquete predefinido para
										poder agregar componentes
									</p>
								)}
							</div>
							<div className="p-6">
										...
										>
											<div className="flex-1">
												...
												<div className="flex items-center gap-4 text-sm text-gray-500">
													...
													<span>
														Precio: $
														{component.basePrice.toLocaleString("es-CL")} CLP
													</span>
												</div>
											</div>
											<div className="flex items-center gap-2 mt-3 sm:mt-0">
												<button
													...
													title={
														selectedPackage
															? "Agregar componente"
															: "Primero selecciona un paquete predefinido"
													}
												>
											...
											<div className="flex items-center justify-between mb-2">
												...
												<span
													className={`px-2 py-1 text-xs rounded-full ${
														package_.complexity === "basic"
															? "bg-green-100 text-green-800"
															: package_.complexity === "standard"
																? "bg-blue-100 text-blue-800"
																: package_.complexity === "advanced"
												...
												selectedPackage?.id === package_.id && (
													<div className="mt-4 p-4 bg-gray-50 rounded-lg">
														...
														<div className="space-y-2">
															{package_.components.map((component) => (
																<div
																	key={component.id}
														...
														<div className="mt-3 pt-3 border-t border-gray-200">
															<p className="text-sm text-gray-600">
																...
																{getEstimatedDuration(package_.complexity)}
															</p>
															<p className="text-sm text-gray-600">
																<strong>Descripción:</strong>{" "}
																{getComplexityDescription(package_.complexity)}
															</p>
														</div>
													</div>
												)}
							...
							<div className="p-6">
								{configurations.length > 0 ? (
											...
											return (
												...
												>
													<div className="flex-1">
														...
														<p className="text-sm text-gray-600">
															${config.unitPrice.toLocaleString("es-CL")} x{" "}
															{config.quantity} {component.unit}
														</p>
													</div>
													<div className="flex items-center gap-2">
														<button
												...
												<button
													...
													onClick={async () => {
														try {
															...
															console.error(
																"Error al continuar con la cotización:",
																error,
															);
														}
													}}
									...
									<div className="text-center py-8">
										<div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
											...
											<div className="mt-3 text-xs text-blue-600">
												<p>
													• {selectedPackage.components.length} componentes
													incluidos
												</p>
												<p>
													• Duración:{" "}
													{getEstimatedDuration(selectedPackage.complexity)}
												</p>
												<p>
													• Complejidad:{" "}
													{getComplexityDescription(selectedPackage.complexity)}
												</p>
												<p className="mt-2 text-blue-700 font-medium">
													💡 Haz clic en "Agregar Componentes del Paquete" para
													personalizar
												</p>
											</div>
											<div className="mt-4 flex flex-col gap-3">
												<button
													...
													onClick={() => {
														console.log(
															'Botón "Agregar Componentes del Paquete" presionado',
														);
									...
									<div className="text-center py-8 text-gray-500">
										...
										<p className="text-sm">
											Agrega componentes o selecciona un paquete
										</p>
									</div>
								)}
							</div>
