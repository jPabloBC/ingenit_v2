...
export default function AdminLogin() {
	...
	const handleLocalStorageLogin = async () => {
		// Local fallback removed to prevent hard-coded/demo users.
		setError(
			"Modo de desarrollo deshabilitado. Configura Supabase para iniciar sesión.",
		);
	};
		...
		try {
			if (isSupabaseConfigured()) {
				try {
					...
					if (error) {
						...
						if (error.message.includes("Invalid login credentials")) {
							...
							setError(
								"Error de conexión con Supabase. Usando modo de desarrollo...",
							);
					...
					if (data.user) {
						// Nota: validación de status en `cn` omitida aquí porque IngenIT gestiona acceso vía tablas `rt_*`.
						// Si se necesita validación centralizada, implementar endpoint específico de IngenIT.
						...
						if (!profile || !["admin", "dev"].includes(profile.role)) {
							if (profile && (profile.role === "user" || profile.role === "viewer")) {
								// Buscar la primera pantalla permitida
								const allowed = Array.isArray(profile.allowed_screens)
									? profile.allowed_screens
									: [];
								if (allowed.length === 0) {
									await supabase.auth.signOut();
									setError(`No tienes pantallas autorizadas. Tu rol es: ${profile.role}`);
									return;
								}
								...
								if (screenError || !screen) {
									await supabase.auth.signOut();
									setError(
										`No se pudo obtener la pantalla autorizada. Tu rol es: ${profile.role}`,
									);
									return;
								}
								setSuccess("¡Login exitoso! Redirigiendo...");
								...
								await supabase.auth.signOut();
								setError(
									`No tienes permisos. Tu rol es: ${profile ? profile.role : "desconocido"}`,
								);
								return;
							}
						}
				...
				if (email === "gerencia@ingenit.cl" && password === "admin123") {
					...
					setLoginAttempts((prev) => prev + 1);
					setError(
						"Credenciales inválidas. Usa: gerencia@ingenit.cl / admin123",
					);
				}
			}
		} finally {
		...
		try {
			if (isSupabaseConfigured()) {
				...
				if (error) {
					...
					setTimeout(() => {
						router.push(
							`/admin/reset-sent?email=${encodeURIComponent(resetEmail)}`,
						);
					}, 2000);
				}
			} else {
