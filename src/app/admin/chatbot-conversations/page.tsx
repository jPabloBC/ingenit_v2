	...
	const fetchConversations = useCallback(async () => {
		try {
			...
			const conversationsArray: Conversation[] = Array.from(
				...
				const contactMessage = sortedMessages.find(
					(msg) => msg.contact_name || msg.contact_phone || msg.contact_email,
				);
		...
		<div className="min-h-screen bg-gray-50 p-4 sm:p-6">
			<div className="max-w-7xl mx-auto xs-screen-optimized">
				...
				<div className="grid grid-cols-1 xl:grid-cols-3 gap-4 sm:gap-6">
					...
					<div
						className={`${selectedConversation ? "xl:col-span-1" : "xl:col-span-3"}`}
					>
								...
								{conversations.length === 0 ? (
									...
									conversations.map((conversation) => (
										<button
											...
											className={`conversation-list-item conversation-transition w-full p-3 sm:p-4 border-b border-gray-100 cursor-pointer hover:bg-gray-50 text-left ${
					...
					<div
						className={`${selectedConversation ? "xl:col-span-2" : "hidden xl:block"}`}
					>
						{selectedConversation ? (
							<div className="bg-white rounded-lg shadow-sm border border-gray-200">
										...
										{selectedConversation.messages.map((message) => (
											...
											>
												...
												>
													...
													<div
														className={`message-bubble px-3 py-2 sm:px-4 sm:py-2 rounded-2xl ${
															message.sender === "user"
																? "bg-blue-600 text-white"
																: "bg-gray-100 text-gray-800"
														}`}
													>
														<p className="message-text text-xs sm:text-sm whitespace-pre-line">
															{message.message}
														</p>
