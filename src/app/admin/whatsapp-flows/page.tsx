...
export default function WhatsAppFlowsPage() {
		...
		try {
			...
			if (error) {
				...
				if (error.code === "PGRST116" || error.message?.includes("404")) {
					console.log(
						"La tabla 'whatsapp_flows' no existe. Ejecuta el script SQL para crearla.",
					);
					setFlows([]);
				} else {
					setFlows([]);
				...
				const transformedFlows: WhatsAppFlow[] = (flowsData || []).map(
					(flow) => ({
						id: flow.id,
						name: flow.name,
	...
	const createFlow = async () => {
		try {
			...
			if (error) {
				...
				if (error.code === "PGRST116" || error.message?.includes("404")) {
					alert(
						"Error: La tabla 'whatsapp_flows' no existe. Ejecuta el script SQL para crear la tabla.",
					);
				} else {
			...
			setFlows((prev) =>
				prev.map((flow) =>
					flow.id === updatedFlow.id
						? { ...updatedFlow, validationStatus: "validated" }
						: flow,
				),
			);
	...
	const filteredFlows = flows.filter((flow) => {
		...
		const matchesStatus =
			statusFilter === "all" || flow.validationStatus === statusFilter;
				...
				<div className="p-4 bg-white border-b border-gray-200">
					<div className="flex items-center justify-between">
						<div className="flex items-center gap-4">
							...
							<div>
								<h1 className="text-2xl font-title text-gray-900">
									{selectedFlow.name}
								</h1>
						...
						<div className="flex items-center gap-2">
							<span
								className={`px-2 py-1 rounded-full text-xs font-medium ${getStatusColor(selectedFlow.validationStatus)}`}
							>
								{selectedFlow.validationStatus === "validated"
									? "Validado"
									: selectedFlow.validationStatus === "pending"
										? "Pendiente"
										: "Error"}
							</span>
						</div>
				...
				<div className="bg-white rounded-2xl shadow-lg border border-gray-100 p-12 text-center">
					...
					<div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-6">
						<p className="text-sm text-yellow-800">
							...
							necesites crear la tabla 'whatsapp_flows' en Supabase. Ejecuta el
							script SQL{" "}
							<code className="bg-yellow-100 px-1 rounded">
								create-whatsapp-flows-table.sql
							</code>{" "}
							en tu base de datos.
						</p>
					</div>
						...
						>
							<div className="p-6">
								<div className="flex items-start justify-between mb-4">
									...
									<div className="flex items-center gap-2">
										<span
											className={`px-2 py-1 rounded-full text-xs font-medium flex items-center gap-1 ${getStatusColor(flow.validationStatus)}`}
										>
											{getStatusIcon(flow.validationStatus)}
											{flow.validationStatus === "validated"
												? "Validado"
												: flow.validationStatus === "pending"
													? "Pendiente"
													: "Error"}
										</span>
									</div>
								...
								<div className="flex items-center justify-between pt-4 border-t border-gray-100">
									<div className="flex items-center gap-2">
										<span
											className={`w-2 h-2 rounded-full ${flow.isActive ? "bg-green-500" : "bg-gray-300"}`}
										></span>
