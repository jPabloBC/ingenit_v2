...
async function getDropboxAccessTokenServer() {
	...
	if (!response.ok)
		throw new Error(data.error_description || "Dropbox token refresh failed");
...
export async function POST(req: Request) {
	try {
		...
		if (
			...
			try {
				...
				const tmp = (await dbx.filesGetTemporaryLink({
					path: dropboxPath,
				})) as DropboxTemporaryLinkResponse;
				temporaryLink = tmp.result?.link || null;
			} catch (dropboxErr: unknown) {
				// If Dropbox fails (invalid token, network, etc.), fallback to local storage
				console.error(
					"Dropbox upload failed, falling back to local write:",
					getErrorMessage(dropboxErr),
				);
		...
		if (!temporaryLink) {
			meta.filePath = path.join(BASE_DIR, filename);
		}
		await fs.writeFile(
		...
		if (next) {
			...
			for (const m of metas) {
				const data = JSON.parse(
					await fs.readFile(path.join(BASE_DIR, m), "utf8"),
				) as PrintMeta;
