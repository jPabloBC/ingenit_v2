...
import { findBankByCodeOrName } from "@/lib/bankCatalog";
...
async function getUserId(req: NextRequest) {
	...
	try {
		if (supabase?.auth?.getUser) {
			const maybe = await supabase.auth.getUser(token);
	...
	try {
			...
			if (bankName) {
				...
				if (!catalogItem) {
					return NextResponse.json(
						{
							success: false,
							error: "bankName no está en la lista de bancos permitidos",
						},
						{ status: 400 },
					);
				}
				const { data: createdBank, error: bankErr } = await supabase
					.from("rt_personal_banks")
					.insert({ name: catalogItem.name, user_id: user_id })
					.select("*")
					.single();
				if (bankErr)
					return NextResponse.json(
				...
				return NextResponse.json(
					{ success: false, error: "bankId o bankName requerido" },
					{ status: 400 },
				);
			}
		} else {
			// verify bank belongs to user or is public
			const { data: bankCheck, error: bankCheckErr } = await supabase
				.from("rt_personal_banks")
				.select("user_id")
				.eq("id", finalBankId)
				.single();
			if (bankCheckErr || !bankCheck)
				return NextResponse.json(
					{ success: false, error: "Banco no encontrado" },
					{ status: 400 },
				);
			if (bankCheck.user_id && bankCheck.user_id !== user_id)
				return NextResponse.json(
					{ success: false, error: "No autorizado para usar este banco" },
					{ status: 403 },
				);
		}

		// Validate that the chosen account type is allowed for this bank (if catalog info exists)
		const bankRow = await supabase
			.from("rt_personal_banks")
			.select("id, name")
			.eq("id", finalBankId)
			.single();
		...
		if (catalogEntry && type && !catalogEntry.types.includes(type)) {
			return NextResponse.json(
				{
					success: false,
					error: `Tipo de cuenta '${type}' no permitido para banco ${catalogEntry.name}`,
				},
				{ status: 400 },
			);
		}
		...
		const { data: createdAccount, error: accountErr } = await supabase
		...
		if (accountErr)
			return NextResponse.json(
				{ success: false, error: accountErr.message },
				{ status: 500 },
			);
		...
		if (initial_tx?.monto) {
			...
			if (txErr) {
				...
				return NextResponse.json(
					{ success: false, account: createdAccount, error: txErr.message },
					{ status: 500 },
				);
			}
			...
			const delta =
				txPayload.tipo === "income"
					? Number(txPayload.monto)
					: -Number(txPayload.monto);
			const { data: updatedAccount, error: updErr } = await supabase
				.from("rt_personal_accounts")
				.update({ balance: (createdAccount.balance || 0) + delta })
