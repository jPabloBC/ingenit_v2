...
if (!SUPABASE_URL || !SUPABASE_ANON_KEY) {
	throw new Error("Missing Supabase env vars for process-leads route");
}
...
export async function POST(_req: NextRequest) {
	try {
		...
		for (const conversation of conversations) {
			...
			// Calcular lead score
			const usersCount = parseInt(conversation.users_count, 10) || 0;
			...
			if (budgetRange.includes("$50,000") || budgetRange.includes("$100,000")) {
				...
				budgetRange.includes("$20,000") ||
				budgetRange.includes("$30,000")
			) {
				leadScore += 25;
			} else if (
				budgetRange.includes("$10,000") ||
				budgetRange.includes("$15,000")
			) {
				leadScore += 20;
			} else if (
				budgetRange.includes("$5,000") ||
				budgetRange.includes("$8,000")
			) {
				leadScore += 15;
			} else if (
				budgetRange.includes("$2,000") ||
				budgetRange.includes("$3,000")
			) {
				leadScore += 10;
			}
			...
			if (
				timeline.includes("1 mes") ||
				timeline.includes("2 meses") ||
			...
			if (
				solutionType.includes("aplicación") ||
				solutionType.includes("app") ||
				solutionType.includes("software")
			) {
				leadScore += 20;
			} else if (
				solutionType.includes("automatización") ||
				solutionType.includes("chatbot")
			) {
				leadScore += 15;
			...
			if (leadError) {
				console.error(
					`Error creando lead para sesión ${conversation.session_id}:`,
					leadError,
				);
				continue;
			}
