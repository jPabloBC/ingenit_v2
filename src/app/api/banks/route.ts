		...
		return NextResponse.json(
			{
				...
				debug: process.env.NODE_ENV !== "production" ? debugErr : undefined,
			},
			{ status: 500 },
		);
	}
...
export async function PATCH(req: NextRequest) {
	try {
		...
		const { data: bankExisting, error: bankErr } = await supabase
			.from("rt_personal_banks")
			.select("user_id")
			.eq("id", id)
			.single();
		if (bankErr || !bankExisting)
			return NextResponse.json(
				{ success: false, error: "Banco no encontrado" },
				{ status: 400 },
			);
		if (bankExisting.user_id && bankExisting.user_id !== user_id)
			return NextResponse.json(
				{ success: false, error: "No autorizado" },
				{ status: 403 },
			);
