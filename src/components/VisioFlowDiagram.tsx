...
type CustomNodeData = ProcessStep & {
	type: string;
};
...
export default function VisioFlowDiagram({
	...
	const getStepType = useCallback(
		(stepName: string, currentProcessType: string): string => {
			const name = stepName.toLowerCase();

			if (currentProcessType === "whatsapp") {
				if (name.includes("análisis") || name.includes("requerimientos"))
					return "start";
				if (name.includes("menú") || name.includes("interactivo"))
					return "whatsapp";
				if (name.includes("api") || name.includes("webhook")) return "api";
				if (name.includes("test") || name.includes("prueba")) return "test";
				if (name.includes("despliegue") || name.includes("activación"))
					return "deploy";
				if (name.includes("documentación")) return "document";
				if (name.includes("entrenamiento")) return "user";
			}

			if (currentProcessType === "web") {
				if (name.includes("análisis") || name.includes("requerimientos"))
					return "start";
			...
			if (currentProcessType === "network") {
				if (name.includes("evaluación") || name.includes("sitio"))
					return "start";
				if (name.includes("cableado") || name.includes("instalación"))
					return "network";
				if (name.includes("configuración")) return "settings";
				if (name.includes("prueba") || name.includes("conectividad"))
					return "test";
				if (name.includes("entrega") || name.includes("capacitación"))
					return "user";
			}

			if (name.includes("inicio") || name.includes("start")) return "start";
			if (name.includes("decisión") || name.includes("decision"))
				return "decision";
			if (name.includes("base de datos") || name.includes("database"))
				return "database";
			if (name.includes("api") || name.includes("servicio")) return "api";
			if (name.includes("test") || name.includes("prueba")) return "test";
			if (name.includes("despliegue") || name.includes("deploy"))
				return "deploy";
			if (name.includes("documentación") || name.includes("manual"))
				return "document";
	...
	const getEdgeColor = useCallback((status: string): string => {
		switch (status) {
			case "completed":
				return "#10b981";
			case "in_progress":
