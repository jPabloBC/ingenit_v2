/* biome-ignore-all lint/suspicious/noExplicitAny: visual flow editor stores heterogeneous node payloads pending strict schema typing */
"use client";
...
const WhatsAppMenuNode = ({
	...
	const removeOption = (optionId: string) => {
		const updatedData = {
			...editData,
			options: editData.options.filter(
				(opt: WhatsAppOption) => opt.id !== optionId,
			),
		};
		setEditData(updatedData);
	};
		...
		>
			...
			{editData.options?.map((option: WhatsAppOption, index: number) => {
				...
				if (isEditing) {
					...
					const topPosition =
						baseTop + index * (optionHeight + 6) + optionHeight / 2 - 5;

					return (
						<div
							key={`handle-container-${option.id}`}
					...
					const baseTop =
						headerHeight + titleHeight + messageHeight + optionPadding;
					...
					const optionTotalHeight =
						optionPadding + optionContentHeight + optionPadding; // padding + contenido + padding
					const topPosition =
						baseTop +
						index * (optionTotalHeight + 6) +
						optionContentHeight / 2 -
						5;
			...
			<div className="flex items-center justify-between mb-3">
				<div className="flex items-center gap-2">
					...
					{data.options &&
						data.options.filter((opt: WhatsAppOption) => opt.nextNodeId)
							.length > 0 && (
							...
							{editData.options.map((option: WhatsAppOption, index: number) => (
								...
								>
									...
									<select
										...
										onChange={(e) =>
											updateOption(option.id, "action", e.target.value)
										}
									...
									<input
										...
										onChange={(e) =>
											updateOption(option.id, "nextNodeId", e.target.value)
										}
				...
				<div className="space-y-2">
					...
					<div className="space-y-2">
						{data.options.map((option: WhatsAppOption, index: number) => (
							<div
								key={option.id}
								className="flex items-center gap-2 text-xs p-2 bg-gray-50 rounded border-l-4 border-green-300"
							>
...
export default function WhatsAppFlowEditor({
	...
	const [processTracker, setProcessTracker] = useState<
		Array<{
			id: string;
			step: string;
	...
	const updateConnectionValidator = useCallback(() => {
		...
		const updatedConnections = newConnections.map((newConn) => {
			const existingConn = connectionValidator.find(
				(conn) => conn.id === newConn.id,
			);
			return existingConn
				? { ...newConn, status: existingConn.status }
				: newConn;
		});
	...
	const initialNodes = useMemo(() => {
		...
		if (flow.startNode) {
			...
			const startNodeData =
				flow.menus.find((m) => m.id === flow.startNode) ||
		...
		flow.decisions?.forEach((decision) => {
			nodes.push({
				...
				position: decision.position
					? { x: decision.position.x, y: decision.position.y }
					: { x: 250, y: 350 },
				data: {
					...decision,
					onUpdate: (updatedData: any) => {
		...
		flow.delays?.forEach((delay) => {
			nodes.push({
				...
				position: delay.position
					? { x: delay.position.x, y: delay.position.y }
					: { x: 250, y: 450 },
				data: {
					...delay,
					onUpdate: (updatedData: any) => {
			...
			flow.menus.forEach((menu) => {
				menu.options.forEach((option, optionIndex) => {
					if (option.nextNodeId) {
						...
						const targetNode =
							flow.menus.find((m) => m.id === option.nextNodeId) ||
	...
	}, [edges.length, updateConnectionValidator]);
	...
	const onConnect = useCallback(
		(params: Connection) => {
			...
			if (params.sourceHandle?.startsWith("option-")) {
		...
		[setEdges, nodes, setNodes, updateConnectionValidator],
	);
	...
	const onEdgeClick = useCallback(
		(_event: any, edge: Edge) => {
			...
			if (newStatus === "1" || newStatus === "2" || newStatus === "3") {
				let _status: "pass" | "error" | "pending";
				let style: any;
				let label: string;
				let labelStyle: any;
	...
	const updateConnectionsFromMenu = (
		menuId: string,
		options: WhatsAppOption[],
	) => {
	...
	const validateFlow = () => {
		...
		if (endNodes.length === 0) {
			...
			results.push({
				type: "success",
				message: `${endNodes.length} nodos de fin encontrados`,
			});
		}
		...
		menuNodes.forEach((menuNode, index) => {
			...
			if (!menuData.title || menuData.title.trim() === "") {
				results.push({
					type: "error",
					message: `Menú ${index + 1}: Falta título`,
				});
			} else {
				results.push({
					type: "success",
					message: `Menú ${index + 1}: Título válido`,
				});
			}
			...
			if (!menuData.message || menuData.message.trim() === "") {
				results.push({
					type: "error",
					message: `Menú ${index + 1}: Falta mensaje`,
				});
			} else {
				results.push({
					type: "success",
					message: `Menú ${index + 1}: Mensaje válido`,
				});
			}
			...
			if (!menuData.options || menuData.options.length === 0) {
				results.push({
					type: "error",
					message: `Menú ${index + 1}: Sin opciones`,
				});
			} else {
				results.push({
					type: "success",
					message: `Menú ${index + 1}: ${menuData.options.length} opciones`,
				});
			}
		});
		...
		messageNodes.forEach((messageNode, index) => {
			...
			if (!messageData.message || messageData.message.trim() === "") {
				results.push({
					type: "error",
					message: `Mensaje ${index + 1}: Falta contenido`,
				});
			} else {
				results.push({
					type: "success",
					message: `Mensaje ${index + 1}: Contenido válido`,
				});
			}
		});
		...
		decisionNodes.forEach((decisionNode, index) => {
			...
			if (!decisionData.question || decisionData.question.trim() === "") {
				results.push({
					type: "error",
					message: `Decisión ${index + 1}: Falta pregunta`,
				});
			} else {
				results.push({
					type: "success",
					message: `Decisión ${index + 1}: Pregunta válida`,
				});
			}
			...
			if (!decisionData.options || decisionData.options.length === 0) {
				results.push({
					type: "error",
					message: `Decisión ${index + 1}: Sin opciones`,
				});
			} else {
				results.push({
					type: "success",
					message: `Decisión ${index + 1}: ${decisionData.options.length} opciones`,
				});
			}
		});
		...
		delayNodes.forEach((delayNode, index) => {
			...
			if (!delayData.duration || delayData.duration < 1) {
				results.push({
					type: "error",
					message: `Espera ${index + 1}: Duración inválida`,
				});
			} else {
				results.push({
					type: "success",
					message: `Espera ${index + 1}: Duración válida (${delayData.duration}s)`,
				});
			}
		});
		...
		if (edges.length === 0) {
			results.push({
				type: "warning",
				message: "No hay conexiones entre nodos",
			});
		} else {
			results.push({
				type: "success",
				message: `${edges.length} conexiones válidas`,
			});
		}
		...
		if (isolatedNodes.length > 0) {
			results.push({
				type: "warning",
				message: `${isolatedNodes.length} nodos sin conexiones`,
			});
		}
	...
	const saveFlow = () => {
		...
		const getOriginalLabel = (edge: Edge): string => {
			const label = typeof edge.label === "string" ? edge.label : "";
			return (
				label.replace(/^[✅❌⏳]\s*(PASS|ERROR|PENDING)\s*-\s*/, "") ||
				"Conexión"
			);
		};
		...
		const updatedFlow: WhatsAppFlow = {
			...
			decisions: decisionNodes.map((n) => {
				...
				return {
					...
					id: n.id,
					question:
						(n.data as any).question ||
						existingDecision?.question ||
						"¿Pregunta?",
			...
			delays: delayNodes.map((n) => {
				...
				return {
					...
					message:
						(n.data as any).message || existingDelay?.message || "Esperando...",
					position: { x: n.position.x, y: n.position.y }, // Actualizar posición
				};
			}),
		...
		console.log(
			"💾 Guardando flujo preservando datos existentes:",
			updatedFlow,
		);
		onSave(updatedFlow);
								...
								<button
									...
									onClick={() => {
										...
										validationProcesses.push({
											id: `start-${Date.now()}`,
											step: "Nodo de Inicio",
											status: (startNode ? "pass" : "error") as
												| "pending"
												| "pass"
												| "error",
											message: startNode
												? "✅ Nodo de inicio encontrado"
												: "❌ Falta nodo de inicio",
											timestamp: new Date().toISOString(),
										});

										// Validar menús
										const menuNodes = nodes.filter(
											(n) => n.type === "whatsappMenu",
										);
										menuNodes.forEach((menu: Node, index) => {
											const menuData = menu.data as any;
											const hasTitle =
												menuData?.title && menuData.title.trim() !== "";
											const hasMessage =
												menuData?.message && menuData.message.trim() !== "";
											const hasOptions =
											...
											validationProcesses.push({
												...
												status:
													hasTitle && hasMessage && hasOptions
														? "pass"
														: "error",
										...
										const connectedOptions = menuNodes.reduce((total, node) => {
											return (
												total +
												((node.data as any).options?.filter(
													(opt: any) => opt.nextNodeId,
												)?.length || 0)
											);
										}, 0);
										...
										validationProcesses.push({
											...
											step: "Conexiones",
											status:
												connectedOptions === totalOptions && totalOptions > 0
													? "pass"
													: "error",
				...
				<div className="absolute top-4 right-4 bg-white rounded-lg shadow-lg p-4 max-w-sm max-h-96 overflow-y-auto">
					...
					<div className="space-y-2">
						{validationResults.map((result) => (
							<div
								key={`${result.type}-${result.message}-${result.timestamp}`}
				...
				{showHelpPanel && (
					<div className="p-4 max-w-sm border-t border-gray-100 max-h-64 overflow-y-auto">
						...
						<div className="text-xs text-gray-600 space-y-1">
							<p>
								🎯 <strong>Puntos de conexión:</strong> Cada opción tiene su
								punto verde en el lado derecho
							</p>
							<p>
								🔗 <strong>Conectar opciones:</strong> Arrastra desde los puntos
								verdes a otros menús
							</p>
							<p>
								📝 <strong>Configurar destino:</strong> Escribe el ID del nodo
								en el campo "ID nodo"
							</p>
							<p>
								💾 <strong>Guardar flujo:</strong> Usa el botón "Guardar flujo"
							...
							<div className="mt-2 p-2 bg-blue-50 rounded border-l-4 border-blue-400">
								...
								<ul className="mt-1 space-y-1">
									<li>
										✅ <strong>PASS:</strong> Cumple - Continúa al siguiente
										paso
									</li>
									<li>
										❌ <strong>ERROR:</strong> No cumple - Finaliza con error
									</li>
									<li>
										⏳ <strong>PENDING:</strong> Pendiente de validación
									</li>
								</ul>
							</div>
							<div className="mt-2 p-2 bg-green-50 rounded border-l-4 border-green-400">
								<p className="text-green-800 font-medium">
									💡 Tip: Mueve los nodos donde quieras y luego guarda para
