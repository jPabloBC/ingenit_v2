import { Calculator, Globe, MapPin, X } from "lucide-react";
import { useState } from "react";
import type { CalculationParams } from "@/lib/serviceCalculations";
...
interface CalculationModalProps {
	service: Service;
	onCalculate: (params: CalculationParams) => Promise<unknown> | undefined;
	onClose: () => void;
	country: string;
	region: string;
}
...
export default function CalculationModal({
		...
		switch (service.id) {
			...
			case "cableado_estructurado":
				return [
					{
						key: "meters",
						label: "Metros de cableado",
						type: "number",
						placeholder: "Ej: 100",
					},
					{
						key: "devices",
						label: "Dispositivos a conectar",
						type: "number",
						placeholder: "Ej: 10",
					},
					{
						key: "points",
						label: "Puntos de configuración",
						type: "number",
						placeholder: "Ej: 20",
					},
				];
			...
			case "monitoreo_red":
				return [
					{
						key: "devices",
						label: "Dispositivos a monitorear",
						type: "number",
						placeholder: "Ej: 20",
					},
					{
						key: "points",
						label: "Alertas a configurar",
						type: "number",
						placeholder: "Ej: 15",
					},
				];
	...
	const CALC_DEBUG = false;
	const calcDebugLog = (...args: unknown[]) => {
		if (CALC_DEBUG) console.log(...args);
	};

	const handleCalculate = async () => {
		calcDebugLog("CalculationModal: iniciar cálculo", {
			service: service.id,
			params,
		});
		try {
				...
				<div className="space-y-4 mb-6">
					<div>
						...
						<p className="text-xs text-gray-500 mt-1">
							Define cuántas unidades comerciales se cotizan (no metros, puntos
							ni usuarios técnicos).
						</p>
					</div>
